 ...
 */
public class PdxDetailActivity extends ForwardActivity {
	...
	private AppPowerBean pdxbean;
	private List<AppPowerComBean> list;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		pdxbean = (AppPowerBean) this.getIntent().getExtras()
				.getSerializable("pdxbean");
		bean = (AppPhyInfoBean) this.getIntent().getExtras()
				.getSerializable("bean");
		Reflex.loadViewForActivityOnCreate(this, R.layout.activity_pdx_detail);
		SysExitUtil.activityList.add(this);
		title.setText("配电箱");
		fillSpinnerAdapter(SpinnerUtilBean.pdxpp(), pinp, "请选择品牌类型");
	...
	private void init() {
		...
		bh.setText(pdxbean.getCode());
		rlStr = pdxbean.getRl() + "";
		rongl.setText(pdxbean.getRl() + "");
		fillSpinnerAdapter(SpinnerUtilBean.pdxpp(), pinp, "请选择品牌类型");
		if (pdxbean.getEwm() != null && !pdxbean.getEwm().equals("")) {
			qrcode.setText(pdxbean.getEwm());
		}
		...
		public void onClick(View arg0) {
			...
			case R.id.save:
				...
				try {
					...
					powerBean.setPhysicCode(bean.getPhysicCode());
					powerBean.setCode(pdxbean.getCode());
					powerBean.setPpCheck(((Option) pinp.getSelectedItem())
							.getValue());
					powerBean.setRlCheck(Double.parseDouble(rongl.getText()
							.toString()));

					powerBean.setCheckUserId(ApplicationData.user.getId() + "");
					powerBean.setEwm(qrcode.getText().toString());
					...
					if (yd_bh1Dtr != null && !yd_bh1Dtr.equals("")) {
						...
						TowerSQliteDbBean.updatePowerComData(db, comBean);
					}
					...
					TowerSQliteDbBean.updatePowerData(db, powerBean);
	...
	private void loadComList() {
		list = new ArrayList<AppPowerComBean>();
		String sql = "select * from ts_power_b where physicCode='"
				+ bean.getPhysicCode() + "' and code='" + pdxbean.getCode()
				+ "'";
		try {
			list = TowerSQliteDbBean.queryPowerComData(mOpenHelper, sql);
	 ...
	 * @author: 罗然
	 * @return void 返回类型
	 * @version: 2015-7-20 上午10:08:36
	 */
	private void loadPowerData() {
		for (AppPowerComBean o : list) {
